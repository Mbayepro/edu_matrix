    ...
    try {
      ...
      const statsData: EcoleStats[] = await Promise.all(
        ecoles.map(async (ecole: any) => {
          const [elevesCount, enseignantsCount, classesCount, elevesStatutData, activeUsers] = await Promise.all([
            ...
            supabase.from('eleves').select('statut_paiement').eq('ecole_id', ecole.id),
          ...
          let tauxPaiement = 0
          if (elevesStatutData.data && elevesStatutData.data.length > 0) {
            const payes = elevesStatutData.data.filter((e: any) => e.statut_paiement === 'payé').length
            tauxPaiement = Math.round((payes / elevesStatutData.data.length) * 100)
          }
